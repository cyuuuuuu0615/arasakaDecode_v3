...
@Config
@Autonomous(name = "AUTO RED 遠射9球")
public class testAuto_v9 extends LinearOpMode {
...
//            FIRE_POS_C = (FIRE_POS_C + offset);
            ...
            switch (state) {
                case INIT:
                    ...
                    break;
                case CHECK_READY:
                    if (System.currentTimeMillis() - timer > 100) { setupDisk(currentSlot); state = State.AIM_DISK; timer = System.currentTimeMillis(); }
                    break;
                case AIM_DISK:
                    if (System.currentTimeMillis() - timer > 600) { robot.kickerServo.setPosition(0.8); state = State.KICK; timer = System.currentTimeMillis(); }
                    break;
                case KICK:
                    if (System.currentTimeMillis() - timer > 300) { robot.kickerServo.setPosition(0.0); removeBall(currentSlot); shotsAttempted++; state = State.RETRACT; timer = System.currentTimeMillis(); }
                    break;
                case RETRACT:
                    if (System.currentTimeMillis() - timer > 250) { state = State.INIT; } // 回到 INIT 找下一顆
                    break;
                case DONE:
                    isShootingMode = false;
