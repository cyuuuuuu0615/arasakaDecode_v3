...
@Config
@Autonomous
public class Auto extends LinearOpMode{   //60cm:25unit          1 cm: 5/12 unit
    double lefthandin = 0;
...
//__________________________________________________________________________
            ...
            while (action){
                ...
                telemetry.update();
                if(!get){
                    currentTimeget = System.currentTimeMillis() - startTimeget;
...
//                    if (!inRange(colorSensor.red(), 600, 1300)) {
    ...
    public class zhengshouGet3Spec implements Action{
        boolean get = false;
        public ColorSensor colorSensor;
        Servo jiazixiaobi = hardwareMap.get(Servo.class,"servo2");
        Servo shoubi = hardwareMap.get(Servo.class,"servo6");
        ...
        public  boolean run(@NonNull TelemetryPacket telemetryPacket){
            ...
            frontLeftMotor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
            boolean action = true;
            zuoshoubi.setPosition(zuoshouhanddon -0.02);
            youshoubi.setPosition(youshouhanddon -0.02);
            float hsvValues[] = {0F,0F,0F};
